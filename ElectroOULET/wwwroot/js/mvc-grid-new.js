...
var MvcGrid = (function () {
    ...
    MvcGrid.prototype = {
        ...
        set: function (options) {
            ...
            if (grid.sourceUrl) {
                ...
                if (options.query !== undefined) {
                    grid.query = options.query;
                } else if (urlsParts[1] || !grid.query) {
                    grid.query = urlsParts[1];
                }
            } else if (options.query === undefined) {
                grid.query = window.location.search;
            } else {
                grid.query = options.query;
            }
        ...
        reload: function () {
            ...
            if (grid.sourceUrl) {
                grid.startLoading(function (result) {
                    var parent = grid.element.parentNode;
        ...
        startLoading: function (success, error) {
            ...
            $.ajax({
                type: "POST",
                url: $("#_subdirectory_").val() + grid.sourceUrl,
                data: { query: (grid.query ? grid.query.replace("query=", "") : "[{}]") },
                success,
                error
            });
            ...
            //grid.request.ontimeout = function () {
            //    if (200 <= grid.request.status && grid.request.status < 400) {
            //        success(grid.request.responseText);
            //    } else if (error) {
            //        error(grid.request.responseText);
