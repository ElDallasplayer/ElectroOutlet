...
var MvcGrid = (function () {
    function MvcGrid(element, options) {
        ...
        grid.sourceUrl = (grid.element.dataset.sourceUrl == undefined ? options.sourceUrl : grid.element.dataset.sourceUrl);
    ...
    MvcGrid.prototype = {
        ...
        set: function (options) {
            ...
            if (options.datos)
                grid.query.datos = options.datos;
        ...
        reload: function () {
            ...
            if (grid.sourceUrl) {
                grid.startLoading(function (result) {
                    var parent = $(grid.element).parent()[0];
                    var i = [].indexOf.call(parent.children, grid.element);

                    $(parent).find(".gridError").html("");
                    ...
                    var newGrid = new MvcGrid(parent.children[i], {
                        ...
                        datos: grid.query.datos,
                        id: grid.element.dataset.id,
                        sourceUrl: grid.sourceUrl,
                        filters: grid.filters
                    });
                    UpdateButtons();
                    grid.dispatchEvent('reloadend', { grid: newGrid });
                }, function (result) {
        ...
        startLoading: function (success, error) {
            ...
            //var _url = (grid.sourceUrl.startsWith($("#_subdirectory_").val()) == undefined ? grid.sourceUrl : $("#_subdirectory_").val() + grid.sourceUrl)
            //    + (grid.query.toString() ? grid.query.toString() + '&' : '?') + '_=' + Date.now();

            var _url = (grid.sourceUrl.startsWith($("#_subdirectory_").val()) == undefined ? grid.sourceUrl : grid.sourceUrl)
                + (grid.query.toString() ? grid.query.toString() + '&' : '?') + '_=' + Date.now();
        ...
        stopLoading: function () {
            ...
            if (grid.loader) {
                grid.loader.parentNode.removeChild(grid.loader);
            }
        },
        ...
        bind: function () {
            ...
            [].forEach.call(pager.pages, function (page) {
                page.addEventListener('click', function () {
                    ...
                    if (window.MVCGrid_PageChanged)
                        window.MVCGrid_PageChanged(grid);
                    //grid.dispatchEvent('reloadend', { grid: grid, page: page });
                });
            });
...
var MvcGridQuery = (function () {
    function MvcGridQuery(value) {
        if (value) {
            if (value.split("=")[0] == "datos") {
                this.parameters = [];
                this.datos = value.split("=")[1];
            }
            else
    ...
    MvcGridQuery.prototype = {
        ...
        toString: function () {
            return this.parameters ? this.parameters.length ? '?' + this.parameters.join('&') : '' : '';
        }
    };
...
var MvcGridFilter = (function () {
    ...
    MvcGridFilter.prototype = {
        ...
        renderFilter: function (name) {
            ...
            var lang = filter.lang[capitalize(filter.column.filter.name)] || {};
